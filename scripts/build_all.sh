...
<dict>
    ...
    <string>FRAMEWORK_NAME_PLACEHOLDER</string>
    <key>CFBundleIdentifier</key>
    <string>com.permanentstore.FRAMEWORK_NAME_PLACEHOLDER</string>
    ...
    <string>MIN_IOS_VERSION_PLACEHOLDER</string>
</dict>
</plist>
EOF
    
    sed -i '' "s/FRAMEWORK_NAME_PLACEHOLDER/${framework_name}/g" "$framework_dir/Info.plist"
    sed -i '' "s/MIN_IOS_VERSION_PLACEHOLDER/${MIN_IOS_VERSION}/g" "$framework_dir/Info.plist"
    
    cat > "$framework_dir/Modules/module.modulemap" << 'EOF'
framework module FRAMEWORK_NAME_PLACEHOLDER {
    umbrella header "FRAMEWORK_NAME_PLACEHOLDER.h"
    export *
}
EOF
    sed -i '' "s/FRAMEWORK_NAME_PLACEHOLDER/${framework_name}/g" "$framework_dir/Modules/module.modulemap"
}
...
#ifndef FRAMEWORK_NAME_PLACEHOLDER_h
#define FRAMEWORK_NAME_PLACEHOLDER_h

#import <Foundation/Foundation.h>

FOUNDATION_EXPORT double FRAMEWORK_NAME_PLACEHOLDERVersionNumber;
FOUNDATION_EXPORT const unsigned char FRAMEWORK_NAME_PLACEHOLDERVersionString[];
...
EOF
    ...
    sed -i '' "s/FRAMEWORK_NAME_PLACEHOLDER/${framework_name}/g" "$framework_dir/Headers/${framework_name}.h"
}
...
# 清理之前的编译产物
rm -f *.o zsign 2>/dev/null || true

# 获取所有 cpp 文件（使用数组避免 find 的输出问题）
cpp_files=()
